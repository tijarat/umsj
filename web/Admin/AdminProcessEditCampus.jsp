...
int updated = 0;
try(PreparedStatement ps = con.prepareStatement("UPDATE UMS.CAMPUS SET UNI_ID = ?, CMP_ABBERV = ?, CMP_NAME = ?, CMP_PREFIX = ?, FRANCHISE = ?, CMP_CDE = ?, CHALLAN_APPROVAL_IND = ?, SUB_CITY_ID = ?, BUILDING_ID = ?, BUSINESS_UNIT_ID = ? WHERE CMP_ID = ?"))
{
ps.setLong(1, Long.parseLong(uniId));
ps.setString(2, cmpAbbrev);
ps.setString(3, cmpName);
if(cmpPrefix.length() == 0) ps.setNull(4, Types.VARCHAR); else ps.setString(4, cmpPrefix);
ps.setString(5, franchise);
if(cmpCode.length() == 0) ps.setNull(6, Types.VARCHAR); else ps.setString(6, cmpCode);
ps.setString(7, challanApproval);
if(subCityId.length() == 0) ps.setNull(8, Types.NUMERIC); else ps.setLong(8, Long.parseLong(subCityId));
if(buildingId.length() == 0) ps.setNull(9, Types.NUMERIC); else ps.setLong(9, Long.parseLong(buildingId));
if(businessUnitId.length() == 0) ps.setNull(10, Types.NUMERIC); else ps.setLong(10, Long.parseLong(businessUnitId));
ps.setLong(11, Long.parseLong(cmpId));
updated = ps.executeUpdate();
}
if(updated == 0) throw new SQLException("Campus record was not found.");
try(Statement logStmt = con.createStatement()) { adminSession.addLog("UPDATE UMS.CAMPUS CMP_ID=" + cmpId + ", CMP_ABBERV=" + cmpAbbrev + ", CMP_NAME=" + cmpName, logStmt); }
con.commit();
session.setAttribute("flashType", "success");
session.setAttribute("flashMessage", "Campus " + cmpName + " has been updated successfully.");
...
if(errorMessage == null || errorMessage.trim().length() == 0) errorMessage = "Unable to update Campus.";
...
response.sendRedirect("AdminEditCampus.jsp?cmpId=" + (cmpId == null ? "" : cmpId));
}
finally
{
